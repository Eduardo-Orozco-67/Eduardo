...
// Registro para almacenar nombres
struct nomcom{
	char nombre[51];
};

// Arreglo de nombres
struct nomcom arr_nombre[4];
...
int grabarDF(){
	...
	FILE *varch = NULL;
	//  w - write, r - read
	varch = fopen("C:\\Users\\vikig\\OneDrive\\Documentos\\C\\distFin2.txt", "w");
	if( varch == NULL ){
		printf("No se pudo abrir para escritura el archivo \n");
		return 0;
	}
	while(vren < 4){
		   fprintf( varch, "%i|%i|%i|%i|%i|%i|%i|%i|%i|%i\n", 
		   a_registro[vren].cb500,
		   a_registro[vren].cb200,
		   a_registro[vren].cb100,
		   a_registro[vren].cb50,
		   a_registro[vren].cb20,
		   a_registro[vren].cm10,
		   a_registro[vren].cm5,
		   a_registro[vren].cm2,
		   a_registro[vren].cm1,
		   a_registro[vren].cm050
		);
		vren = vren + 1;
	} // fin de while
	// cerrando el archivo
...
void capturaRenglon(){
  	...
  	if(g_renglon < 5){
  		...
  		strcpy(vnombre_c, vnombre);
  		strcat(vnombre_c, " ");
  		strcat(vnombre_c, vapellido);
  		strcat(vnombre_c, "\0");
  		printf("Llenando arreglo nombres completos \n");
  		// almacenar en arreglo nombre_completo
  		// el renglon del arreglo es uno menor al renglon del lab
  		strcpy( arr_nombre[g_renglon-1].nombre, vnombre_c );
...
void main(){
	...
	while (vsalir == 0){
		...
		printf("12. Grabar arreglo BiDim a un archivo \n");
		vopc = 0;
		printf("Opcion: ");
		scanf("%i", &vopc);
		switch(vopc){
